...
export function isValidUrl(url: string): boolean {
    try {
        ...
        const isTrustedHost = parsed.hostname === 'evzone.com' || parsed.hostname.endsWith('.evzone.com') ||
                              parsed.hostname === 'evzone.app' || parsed.hostname.endsWith('.evzone.app');
        return ['https:', 'http:'].includes(parsed.protocol) && isTrustedHost;
    } catch {
        return false;
    }
}
...
export function sanitizeUrl(url: string): string {
    ...
    try {
        ...
        const isEvzoneHost = parsed.hostname === 'evzone.com' || parsed.hostname.endsWith('.evzone.com') ||
                             parsed.hostname === 'evzone.app' || parsed.hostname.endsWith('.evzone.app');
        if (parsed.protocol !== 'https:' && isEvzoneHost) {
