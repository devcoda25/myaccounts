...
export function sanitizeUrl(url: string): string {
    ...
    try {
        ...
        // Reject non-HTTP/HTTPS schemas to prevent XSS (e.g. javascript:)
        if (!['https:', 'http:'].includes(parsed.protocol)) {
            return '';
        }

        // Only allow HTTPS for production domains
        if (parsed.protocol === 'http:' && (parsed.hostname.includes('evzone.com') || parsed.hostname.includes('evzone.app'))) {
