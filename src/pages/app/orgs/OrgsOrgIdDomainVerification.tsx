...
/**
 * EVzone My Accounts - Domain Verification
 * Route: /app/orgs/:orgId/domain-verification
 *
 * Features:
 * • Show required DNS TXT record
 * • Verify status check
 * • Security warnings
 */
...
function randomToken() {
  return `evz-${Math.random().toString(16).slice(2, 10)}-${Math.random().toString(16).slice(2, 8)}`;
}
...
export default function DomainVerificationPage() {
    ...
    return [
      {
        ...
        token,
        recordName: `_evzone-verify.evworld.africa`,
        recordValue: `evzone-domain-verification=${token}`,
        status: "Verified",
        lastCheckedAt: Date.now() - 1000 * 60 * 14,
      },
      {
        id: "dom2",
        domain: "evzonecharging.com",
        token: randomToken(),
        recordName: `_evzone-verify.evzonecharging.com`,
        recordValue: "evzone-domain-verification=evz-xxxx",
        status: "Pending",
        lastCheckedAt: Date.now() - 1000 * 60 * 40,
      },
    ];
  });

  const [activeId, setActiveId] = useState<string>("dom2");
  ...
  const addDomain = () => {
    ...
    const token = randomToken();
    const id = `dom_${Math.random().toString(16).slice(2, 7)}`;
    const item: DomainItem = {
      id,
      domain: d,
      token,
      recordName: `_evzone-verify.${d}`,
      recordValue: `evzone-domain-verification=${token}`,
      status: "Not started",
  ...
  const startVerification = () => {
    if (!canEdit) {
      setSnack({ open: true, severity: "warning", msg: "Only admins can start verification." });
      return;
    }
  ...
  const verifyNow = async () => {
    ...
    setSnack({ open: true, severity: "info", msg: "Checking DNS (demo)..." });

    await new Promise((r) => setTimeout(r, 650));

    // Demo logic: domains containing "ev" verify, others fail once then allow retry.
    const shouldPass = active.domain.includes("ev") || Math.random() > 0.4;
    ...
    setDomains((prev) =>
      prev.map((d) =>
        d.id === active.id
          ? {
            ...d,
            status: shouldPass ? "Verified" : "Failed",
    ...
    setSnack({ open: true, severity: shouldPass ? "success" : "error", msg: shouldPass ? "Domain verified." : "DNS record not found yet. Try again." });
  };
  ...
  const removeDomain = (id: string) => {
    ...
    setSnack({ open: true, severity: "success", msg: "Domain removed (demo)." });
  };
  ...
  const goSso = () => setSnack({ open: true, severity: "info", msg: "Navigate to /app/orgs/:orgId/sso (demo)." });
          ...
          <Box className="mx-auto max-w-6xl px-4 py-3 md:px-6">
            <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={2}>
              ...
              <Stack direction="row" spacing={1} alignItems="center">
                <TextField select size="small" label="My role" value={canEdit ? "Admin" : "Member"} onChange={() => { }} sx={{ minWidth: 140, display: { xs: "none", md: "block" } }}>
